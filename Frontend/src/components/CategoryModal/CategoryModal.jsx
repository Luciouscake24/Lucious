...
const CategoryModal = ({ category, closeModal }) => {
  ...
  const categoryCollections = collections.filter(
    col => col.categorySlug === category.slug || col.categoryId === category._id
  );
...
if (selectedCollection) {
  collectionProducts = products.filter(
    p => p.collectionId === selectedCollection.slug
  );
  ...
  if (sort === "low") collectionProducts.sort((a,b)=>a.price-b.price);
  if (sort === "high") collectionProducts.sort((a,b)=>b.price-a.price);
}

  /* ================= CART ================= */
  const handleAddFirstTime = (cake) => {
    setQty(prev => ({ ...prev, [cake._id]: 1 }));
    dispatch({ type: "ADD_TO_CART", payload: { ...cake, quantity: 1 } });
  };
    ...
    <div className="modal-overlay" onClick={closeModal}>
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          ...
          <X onClick={closeModal} className="close-btn" />
        </div>
          ...
          <div className="modal-products">
            ...
            {categoryCollections.map((col) => (
              ...
              >
                ...
                <h3>{col.name}</h3>
                <button className="add-btn">View Cakes</button>
              </div>
            ))}
          </div>
        )}
          ...
          <>
            ...
            <div className="modal-products">
              ...
              {collectionProducts.map((cake) => (
                <div className="modal-card" key={cake._id}>
                  ...
                  {!qty[cake._id] ? (
                    ...
                    <div className="qty">
                      <button onClick={() => changeQty(cake._id, "dec")}>-</button>
                      <span>{qty[cake._id]}</span>
                      <button onClick={() => changeQty(cake._id, "inc")}>+</button>
                    </div>
                  )}
